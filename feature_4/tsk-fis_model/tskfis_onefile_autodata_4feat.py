...
def infer_num_mf_for_feature(series: pd.Series) -> int:
    """
    Decide number of MFs based on unique values in the ORIGINAL (unscaled) dataset column.

    Rule:
    - If binary (0/1) or (1/2): use 2 MFs
    - Else: use 3 MFs
    """
...
def main():
    ...
    # Module weight mode:
    # - "raw": unconstrained learnable weights
    # - "softmax": weights sum to 1 (easier to present as importance)
    WEIGHT_MODE = "raw"

    # Only use these 4 features (as requested)
    REQUESTED_FEATURES = ["age_years", "ap_hi", "ap_lo", "cholesterol"]

    set_seed(SEED)

    os.makedirs(OUT_ROOT, exist_ok=True)
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = os.path.join(OUT_ROOT, f"tskfis_run_{run_id}")
    os.makedirs(out_dir, exist_ok=True)
    ...
    missing = [c for c in [LABEL_COL] + REQUESTED_FEATURES if c not in df_train.columns]
    if missing:
        raise ValueError(f"Missing columns in train.csv: {missing}")

    # Keep feature order
    feature_cols = [c for c in REQUESTED_FEATURES if c in df_train.columns]
    if len(feature_cols) != len(REQUESTED_FEATURES):
        raise ValueError(f"Feature columns mismatch. Expected {REQUESTED_FEATURES} but got {feature_cols}.")

    # -------------------------
    # Infer MF counts from RAW data
    # -------------------------
    feature_mf_num = {c: infer_num_mf_for_feature(df_train[c]) for c in feature_cols}
    ...
    # -------------------------
    # Build modules (Modular TSK-FIS)
    # -------------------------
    feature_index_map = {name: i for i, name in enumerate(feature_cols)}

    # Modular grouping for your 4-feature setup:
    # - Bio: age_years
    # - Blood Pressure: ap_hi, ap_lo
    # - Metabolic: cholesterol
    module_specs = [
        ModuleSpec("Bio-Demographics", ["age_years"]),
        ModuleSpec("Blood Pressure", ["ap_hi", "ap_lo"]),
        ModuleSpec("Metabolic", ["cholesterol"]),
    ]
    ...
    model = ModularTSKFIS(modules, device=device, weight_mode=WEIGHT_MODE).to(device)
    ...
    pred_df[LABEL_COL] = y_test.astype(int)
    ...
    val_df[LABEL_COL] = y_val.astype(int)
    ...
    plot_module_scatter(val_df, num_modules, os.path.join(out_dir, "module_scatter_val"), label_col=LABEL_COL)
    ...
    export_step1_to_5(model, modules, module_feature_names, feature_cols,
                      ...
                      device=device, num_samples=5, label_col=LABEL_COL)
    ...
    metrics = {
        ...
        "feature_mf_num_inferred": feature_mf_num,
        ...
        "data_paths": {"train": TRAIN_CSV, "val": VAL_CSV, "test": TEST_CSV},
    }
