...
def timestamp_run_dir(base_outdir: str) -> str:
    ...
    run_dir = os.path.join(base_outdir, f"tskfis_run_weighted_and_threshold{ts}")
    ensure_dir(run_dir)
    return run_dir
...
class ModularTSKFIS(nn.Module):
    def __init__(self, feature_cols, modules, feature_mf_num, device):
        ...
        # learnable module weights (softmax)
    ...
    def init_mfs(self, X_train_z: np.ndarray):
        ...
        """
        for m, mod in enumerate(self.module_models):
            idx = mod.feat_index
            Xz = X_train_z[:, idx]
    ...
    def forward(self, Xz):
        ...
        logit = torch.sum(alpha * module_outs, dim=1)  # [B]
        return logit
...
def make_median_point(train_df: pd.DataFrame, feature_cols):
    ...
    for f in feature_cols:
        if is_discrete_ordinal(train_df[f]):
            # use median as discrete rounded
...
def plot_3d_surfaces_pairwise(run_dir, model, scaler, train_df, feature_cols):
    ...
    for fx, fy in pairs:
        ...
        # if cholesterol, force discrete axis
        if fy == "cholesterol":
...
def train_model(run_dir, model, scaler, X_train_orig, y_train, X_val_orig, y_val):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"[Epoch {epoch:03d}] loss={ep['train_loss']:.4f}  val_auc={val_auc:.4f}  val_acc={val_acc:.4f}  weights={ep['module_weights']}")
...
def main():
    ...
    # Train
    best_path, best_val_auc = train_model(run_dir, model, scaler, X_train, y_train, X_val, y_val)
